...
class V264CheckpointStartupTests(unittest.TestCase):
    ...
    def test_fresh_round_directory_is_created_by_inherited_executor(self):
        completed = run_phoenix(
            """
            import importlib.util
            import sys
            ...
            spec = importlib.util.spec_from_file_location(
                'tdh_v264_fresh_round_ownership', path
            )
            module = importlib.util.module_from_spec(spec)
            sys.modules[spec.name] = module
            spec.loader.exec_module(module)
            ...
            def execute(self, round_number, preflight):
                ...
                assert not round_dir.exists(), (
                    'checkpoint wrapper pre-created the fresh round directory'
                )
                round_dir.mkdir()
                return (
                    {'research_round': round_number, 'verdict': 'REVISE'},
                    False,
                    None,
                )
            module.Controller._v262_execute_round = execute
            try:
                with tempfile.TemporaryDirectory() as directory:
                    controller = object.__new__(module.Controller)
                    controller.run_id = 'v264-fresh-round'
                    controller.run_dir = Path(directory)
                    result = controller.execute_round(
                        1, {'status': 'PREFLIGHT_OK'}
                    )
                    assert result[0]['research_round'] == 1
                    assert calls['count'] == 1
                    round_dir = Path(directory) / 'round-01'
                ...
                module.Controller._v262_execute_round = original
            print('V264_FRESH_ROUND_OWNERSHIP_OK')
            """
        )
        self.assertEqual(completed.returncode, 0, completed.stdout)
        self.assertIn('V264_FRESH_ROUND_OWNERSHIP_OK', completed.stdout)
    ...
    def test_runtime_contract_preserves_offline_fail_closed_policy(self):
        completed = run_phoenix(
            ...
            assert (
                contract['v264_inherited_executor_owns_fresh_round_directory']
                is True
            )
