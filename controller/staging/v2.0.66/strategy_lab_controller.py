#!/usr/bin/env python3
"""TDH Strategy Lab v2.0.65 frontier inbox lifecycle and recovery.
...
def _v265_reviewed_proposal_registry() -> dict[str, tuple[str, ...]]:
    ...
    path = HERE / 'research' / 'frontier-scout-approved-seeds-v1.jsonl'
    if not path.exists():
        return {}
    if path.is_symlink() or not path.is_file():
        raise LabError('v2.0.65 reviewed Scout registry path is unsafe')
    if path.stat().st_size > 1_000_000:
        raise LabError('v2.0.65 reviewed Scout registry exceeds bounded size')

    reviewed: dict[str, list[str]] = {}
    try:
        lines = path.read_text(encoding='utf-8').splitlines()
    except (OSError, UnicodeDecodeError) as exc:
        raise LabError('v2.0.65 reviewed Scout registry is unreadable') from exc
    ...
    for line_number, line in enumerate(lines, start=1):
        ...
        try:
            ...
            raise LabError(
                f'v2.0.65 reviewed Scout registry line {line_number} is invalid'
            ) from exc
        if not isinstance(row, dict):
            raise LabError('v2.0.65 reviewed Scout registry row is not an object')
        ...
        if (
            row.get('registry_id') != 'tdh-controller-reviewed-scout-seeds-v1'
            ...
            raise LabError('v2.0.65 reviewed Scout registry contract drift')
...
def _v265_scout_inbox_lifecycle() -> dict[str, Any]:
    ...
    for path in paths:
        ...
        if approved_experiments:
            ...
            continue
        decision = _v266_read_decision(digest)
