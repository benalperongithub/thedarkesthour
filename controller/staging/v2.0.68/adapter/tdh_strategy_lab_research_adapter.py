...
BASE = Path(
    ...
    'v2.0.67/adapter/tdh_strategy_lab_research_adapter.py'
)
spec = importlib.util.spec_from_file_location('tdh_adapter_v267_for_v268', BASE)
if spec is None or spec.loader is None:
    raise RuntimeError('cannot load sealed v2.0.67 research adapter')
...
kspec = importlib.util.spec_from_file_location(
    'tdh_kernel_v268_adapter', KERNEL_PATH
)
if kspec is None or kspec.loader is None:
    raise RuntimeError('cannot load v2.0.68 research kernel')
...
V268_IMPLEMENTATION = 'VTM40_VOLRANK60_P80_CAUSAL_SHUFFLE100_V1'
V268_MIN_TRADES_PER_SYMBOL = 30
...
def _causal_shuffled_rank(
    ...
    if iterations != 100:
        raise AdapterError('v2.0.68 requires exactly 100 shuffle labels')
...
def strategy_signal(
    ...
    if config.get('experiment_id') not in V268_EXPERIMENT_IDS:
        return _BASE_SIGNAL(frame, config)
    if config.get('family') != V268_FAMILY:
        raise AdapterError('v2.0.68 experiment family drift')
    ...
    if mode == 'PERFORMANCE':
        ...
        raise AdapterError('v2.0.68 unknown control mode')
...
def finalize_comparisons(results: list[dict[str, Any]], stage: str) -> None:
    ...
    for row in results:
        ...
        if not isinstance(metrics, dict):
            raise AdapterError('v2.0.68 performance metrics missing')
        ...
        if trade_count < V268_MIN_TRADES_PER_SYMBOL:
            row['status'] = 'FAIL'
