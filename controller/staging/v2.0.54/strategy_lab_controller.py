#!/usr/bin/env python3
"""TDH Strategy Lab v2.0.54 Frontier Scout + validated replenishment.
...
def _v254_scout_prompt(
    ...
    prompt = (
        ...
        'Return ONLY one JSON object matching tdh-frontier-inbox-v1: proposal_version, hypothesis_id '
        '(TDH-SCOUT plus six digits), status=UNTRUSTED_INBOX, family_thesis, causal_mechanism, '
        'source_evidence (source_id, claim, provenance), required_data, timeframes, bounded_parameters, '
        'baseline_thesis, negative_control_thesis, falsification (failure_condition, minimum_test, '
        'expected_information_gain), safety. Safety must be data_only=true, contains_executable_code=false, '
        'trading_actions=false, exchange_api_access=false, controller_registration_required=true. '
        'Never emit code, commands, API instructions, or claim registration. New families remain untrusted. '
...
class Controller(V246_DISPATCH_BASE):
    ...
    def _run_frontier_scout(
        ...
        args = _critic_args(self)
        workspace = Path(tempfile.mkdtemp(prefix='tdh-v254-scout-', dir='/tmp'))
        workspace.chmod(0o755)
        log = sd / 'claude-frontier-scout.json'
        try:
            self.run_worker(
                ...
                args=args,
                cwd=workspace,
                prompt=_v254_scout_prompt(context, research, critic),
                log_path=log,
                timeout=int(self.config.worker_timeout_seconds),
            )
            usage, model_usage, outer = _claude_raw_usage(log)
            self._avu['claude'] = usum(self._avu.get('claude', {}), usage)
            raw = _extract_critic_payload(outer)
    ...
    def ensure_av(self, rd: Path, c: dict[str, Any]):
        ...
        if not yes:
            ...
            if (
                ...
                and research
                and critic
                and _v254_scout_needed(c)
            ):
                try:
                    scout = self._run_frontier_scout(sd, c, research, critic)
