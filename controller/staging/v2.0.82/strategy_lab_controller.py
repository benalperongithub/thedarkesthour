#!/usr/bin/env python3
"""TDH Strategy Lab v2.0.78 sealed diversification implementation bridge.
...
After the reviewed queue is correctly exhausted, v2.0.61 admits its one exact
controller-reviewed RSI_GATED_REVERSION Packet-A row into the completed round
context. Sealed v2.0.28 nevertheless recomputes historical frontier state only
while constructing the provider JSON output example and cannot see that current
round replenishment. v2.0.75 caches the exact hash-bound Packet-A row and exposes
a deep-copied CHANGE_STRATEGY_FAMILY shape only during that inherited example
builder. The cache cannot select, validate, execute, backtest or promote a
candidate; all identity drift and unknown states remain fail closed.

Runtime acceptance proved the example bridge but also showed that the exact
Packet-A config already exists in authoritative global proposal memory. The
immutable validator rejected it only after one paid proposal and one bounded
repair call. v2.0.76 reuses the same sealed full-history hash reader as v2.0.74
immediately after Packet-A replenishment. It removes only a hash-equal exact
Packet-A row before provider dispatch, leaves novel Packet-A rows unchanged,
and records the decision locally. The inherited empty-frontier path then skips
proposal generation and continues to controller-owned review without changing
validation, S1, promotion, trading or exchange boundaries.

Runtime acceptance then confirmed that seven data-only proposals had reached
``READY_FOR_SEALED_IMPLEMENTATION`` while the raw review backlog continued to
drain. v2.0.77 deterministically selects exactly one of those already-classified
records and emits a bounded implementation packet whose proposal, producer
decision, family, data capability and payload hashes are all controller-bound.
The packet is review input only: it cannot register a family, create executable
code, invoke a provider, enter S1 or promote a strategy. Unknown or inconsistent
state fails closed, while the immutable raw proposal and decisions remain intact.

Runtime acceptance selected proposal ``2069e9...`` as the first exact
VOLUME_TSMOM implementation packet. v2.0.78 binds that packet to three sealed
S1 experiments across 1h, 4h and 1d. The only primary change from the existing
v2.0.68 ablation is universe diversification from three to four high-cap coins;
the proposed return and volume lookback grids remain closed until this cheapest
confound test passes. The candidate, baseline and causal negative control remain
identical, DOGEUSDT remains excluded, and authoritative global memory removes
duplicates before any provider call. No model text becomes executable and no
S1, promotion, trading or exchange boundary is weakened.

v2.0.79 then bound those sealed identities into the local Phoenix adapter and
proved that the four-symbol S1 can execute end to end.  The completed S1 raw
evidence exposed a legacy v2.0.41 post-S1 prompt-headroom edge: the inherited
two-level compactor stopped at 7,255 characters even though all raw evidence
had already been written safely to disk.  v2.0.80 adds one deterministic,
prompt-only fallback for that exact known error.  It preserves every current
candidate's registered config, hard financial metrics, failed control gates,
strongest chronological counterexample, bounded negative memory and provenance hashes while
keeping the analysis packet below a stricter 6,200-character ceiling.  Unknown
errors still fail closed.  Raw evidence, S1 gates, validation, promotion,
provider selection, trading and exchange boundaries are unchanged.

Runtime acceptance of v2.0.80 showed that the full immutable S1 evidence still
contains four chronological folds per performance candidate, while the
inherited prompt summary intentionally removes ``fold_results`` before the
post-S1 compactor runs.  v2.0.81 derives the exact current run and round from
the already-bound candidate identities, reads only that immutable
``S1_FINANCIAL_EVIDENCE.json`` artifact, and carries one hash-verified worst-fold
excerpt per current candidate into the v2.0.80 fallback.  It never scans history,
and it fails closed on run, round, config, candidate or result-hash drift.  No raw
evidence is rewritten; prompt limits, S1 gates, validation, promotion, provider
selection, trading and exchange boundaries remain unchanged.

Runtime acceptance of v2.0.81 then exposed a productivity boundary rather than a
safety one.  Every replenishment lane shipped so far draws from a finite,
hard-coded seed list: three v2.0.68 ablation seeds, three v2.0.78 four-coin
seeds and one v2.0.61 Packet-A row.  Authoritative global memory has consumed
all of them, the inherited selection layer now supplies no family card and no
experiment seed, and the sealed frontier arrives empty after the v2.0.36
structural NO_SIGNAL quarantine.  The lane chain therefore rebuilds an identical
empty frontier every run, skips both providers and ends in
``ROUND_BUDGET_EXHAUSTED`` without any measurable state change.  v2.0.82 adds
one deterministic recovery lane after every inherited lane: candidates are
reconstructed from the sealed kernel registry in a single stable family-then-
experiment order and at most one is admitted.  Nothing is invented; each row is
rebuilt with the sealed kernel schema, hash-bound to its registry entry,
...
V275_PACKET_A_EXAMPLE_BRIDGE_VERSION = (
    'tdh-avenox-packet-a-example-frontier-bridge-v275'
)
V276_PACKET_A_GLOBAL_MEMORY_FILTER_VERSION = (
    'tdh-avenox-packet-a-global-memory-filter-v276'
)
V277_SEALED_IMPLEMENTATION_QUEUE_VERSION = (
    'tdh-avenox-sealed-implementation-queue-v277'
)
V278_DIVERSIFICATION_BRIDGE_VERSION = (
    'tdh-avenox-volume-tsmom-diversification-bridge-v278'
)
V278_REVIEWED_REGISTRY_ID = (
    'tdh-v278-volume-tsmom-diversification-bridge-v1'
)
V278_REVIEWED_SEEDS_FILENAME = (
    'v278-volume-tsmom-diversification-bridge-v1.jsonl'
)
...
V278_REVIEWED_SEED_PRIORITY = (
    'TDH-SCOUT-000001-VTM-VOL80-NODOGE-4COIN-1H',
    'TDH-SCOUT-000001-VTM-VOL80-NODOGE-4COIN-4H',
    'TDH-SCOUT-000001-VTM-VOL80-NODOGE-4COIN-1D',
)
V278_REVIEWED_SYMBOLS = ('BTCUSDT', 'ETHUSDT', 'SOLUSDT', 'XRPUSDT')
V280_POST_S1_HEADROOM_VERSION = 'tdh-avenox-post-s1-headroom-bridge-v280'
V280_POST_S1_ANALYSIS_MAX_CHARS = 6200
V280_LEGACY_HEADROOM_ERROR_PREFIX = (
    'v2.0.41 post-S1 precheck compaction cannot preserve headroom:'
)
V281_POST_S1_FOLD_BRIDGE_VERSION = (
    'tdh-avenox-post-s1-fold-counterexample-bridge-v281'
)
V281_RUNS_ROOT = Path(
    '/srv/tdh-collab/controller/strategy-lab-v2/runs'
)
V281_FINANCIAL_EVIDENCE_FILENAME = 'S1_FINANCIAL_EVIDENCE.json'
V282_LEGAL_FRONTIER_RECOVERY_VERSION = (
    'tdh-avenox-legal-frontier-recovery-v282'
)
V282_RECOVERY_ARTIFACT_FILENAME = 'LEGAL_FRONTIER_RECOVERY_V282.json'
V282_RECOVERY_SCHEMA_VERSION = 1
V282_MAX_REGISTRY_ADMISSIONS = 1
V282_REJECTED_SAMPLE_LIMIT = 12
V282_ELIGIBLE_TIMEFRAMES = V254_SCOUT_TIMEFRAMES
V282_REJECTION_REASONS = (
    'ALREADY_USED_EXPERIMENT_ID',
    'EXCLUDED_PEER_FAMILY',
    'UNSUPPORTED_TIMEFRAME',
    'DUPLICATE_IN_ROUND_CONTEXT',
    'DUPLICATE_IN_AUTHORITATIVE_GLOBAL_MEMORY',
    'NOT_EXACT_REGISTERED_SEED',
    'NO_LEGAL_SINGLE_AXIS_TRANSITION',
)
V282_STATUS_ADMITTED = 'EXACT_REGISTERED_ROTATION_ADMITTED'
V282_STATUS_EXHAUSTED = 'REGISTRY_ROTATION_EXHAUSTED_FAIL_CLOSED'
...
def _v251_legal_frontier_item(
    ...
    if (
        ...
        return False
    if (
        'v278_diversification_queue' in item
        and not _v278_exact_diversification_item(item)
    ):
        return False
    if (
        'v282_registry_rotation' in item
        and not _v282_exact_rotation_item(item)
    ):
        return False
...
def _v275_exact_packet_a_item(item: Any) -> bool:
    """Recognize only the sealed v2.0.61 Packet-A frontier row."""
    ...
    config = item.get('config')
    registration = item.get('v254_registration')
    if not isinstance(config, dict) or registration != {
        'version': V254_FRONTIER_SCOUT_VERSION,
        'source': 'CONTROLLER_REVIEWED_VIDEO_INTAKE_PACKET_A',
        'experiment_id': V261_PACKET_A_EXPERIMENT_ID,
        'family_id': 'RSI_GATED_REVERSION',
    ...
    if (
        ...
        or experiment.get('family_id') != 'RSI_GATED_REVERSION'
        or experiment.get('universe') != ['BTCUSDT', 'ETHUSDT', 'SOLUSDT']
    ):
        return False
    expected = kernel.validate_config(
        kernel.performance_config(experiment, 'BTCUSDT')
    ...
    return (
        config.get('experiment_id') == V261_PACKET_A_EXPERIMENT_ID
        and config.get('family') == 'RSI_GATED_REVERSION'
        and config.get('symbol') == 'BTCUSDT'
        and _v254_canonical_hash(config) == _v254_canonical_hash(expected)
    )
...
def _v278_exact_diversification_item(item: Any) -> bool:
    """Recognize only an exact hash-bound v2.0.78 diversification seed."""
    if not isinstance(item, dict):
        return False
    config = item.get('config')
    queue = item.get('v278_diversification_queue')
    if not isinstance(config, dict) or not isinstance(queue, dict):
        return False
    experiment_id = config.get('experiment_id')
    if experiment_id not in V278_REVIEWED_SEED_PRIORITY:
        return False
    if queue != {
        'version': V278_DIVERSIFICATION_BRIDGE_VERSION,
        'source': 'SEALED_V278_CONTROLLER_REVIEWED_REGISTRY',
        'registry_id': V278_REVIEWED_REGISTRY_ID,
    ...
    return (
        set(reviewed) == set(V278_REVIEWED_SEED_PRIORITY)
        and _v260_exact_controller_registered_seed(item)
    )


def _v278_sealed_diversification_replenishment(
    context: dict[str, Any],
    actor: str,
    root: Path | None,
) -> dict[str, Any]:
    """Admit one novel four-coin bridge after the v2.0.68 queue exhausts."""
    frontier = context.get('novelty_frontier')
    if actor != 'codex' or not isinstance(frontier, list) or frontier:
        return context

    prior_queue = context.get('v269_reviewed_seed_replenishment')
    prior_filter = context.get('v274_global_memory_queue_filter')
    if (
        not isinstance(prior_queue, dict)
        or prior_queue.get('status') != 'REVIEWED_SEED_QUEUE_EXHAUSTED'
        or not isinstance(prior_filter, dict)
        or prior_filter.get('selected_experiment_id') is not None
    ):
        return context
    ...
    if set(reviewed) != set(V278_REVIEWED_SEED_PRIORITY):
        raise LabError('v2.0.78 diversification registry binding drifted')
    ...
    selected_symbol = (
        source_symbol
        if source_symbol in V278_REVIEWED_SYMBOLS
        else V278_REVIEWED_SYMBOLS[0]
    )
    skipped: list[dict[str, Any]] = []
    illegal: list[dict[str, Any]] = []
    skipped_used: list[dict[str, Any]] = []
    event = {
        'version': V278_DIVERSIFICATION_BRIDGE_VERSION,
        'actor': 'codex',
        'registry_id': V278_REVIEWED_REGISTRY_ID,
        ...
        'source_decision_sha256': V278_SOURCE_DECISION_SHA256,
        'source_packet_sha256': V278_SOURCE_PACKET_SHA256,
        'priority': list(V278_REVIEWED_SEED_PRIORITY),
        'symbols': list(V278_REVIEWED_SYMBOLS),
        'primary_change': 'UNIVERSE_COUNT_3_TO_4',
    ...
    for experiment_id in V278_REVIEWED_SEED_PRIORITY:
        if experiment_id in used_experiment_ids:
            # Recorded from v2.0.82 onward. While this exit was silent an
            # exhausted queue could not be reconciled against its own priority
            # list, which is exactly what made the no-frontier loop opaque.
            skipped_used.append({
                'experiment_id': experiment_id,
                'reason': 'ALREADY_USED_EXPERIMENT_ID',
            })
            continue
        experiment = experiments.get(experiment_id)
        if not isinstance(experiment, dict):
            raise LabError('v2.0.78 diversification experiment is missing')
        if (
            experiment.get('registry_id') != V278_REVIEWED_REGISTRY_ID
            or experiment.get('family_id') != 'VOLUME_TSMOM'
            or tuple(experiment.get('universe') or ())
            != V278_REVIEWED_SYMBOLS
        ):
            raise LabError('v2.0.78 diversification experiment drifted')
        admission = experiment.get('controller_admission')
        if (
            not isinstance(admission, dict)
            or admission.get('source_proposal_sha256')
            ...
            or admission.get('source_decision_sha256')
            != V278_SOURCE_DECISION_SHA256
            or admission.get('source_packet_sha256')
            != V278_SOURCE_PACKET_SHA256
            or admission.get('primary_change') != 'UNIVERSE_COUNT_3_TO_4'
            or admission.get('single_material_axis') is not True
            or admission.get('min_unique_coins_required') != 4
            or admission.get('status')
            != 'CONTROLLER_APPROVED_SEALED_REGISTRY'
            or admission.get('s1_only') is not True
            or admission.get('controller_only_promotion') is not True
            ...
            raise LabError('v2.0.78 diversification provenance drifted')

        candidate_config = kernel.validate_config(
            kernel.performance_config(experiment, selected_symbol)
        )
        digest = _v254_canonical_hash(candidate_config)
        if digest in used_config_hashes or digest in historical:
            skipped.append({
                'experiment_id': experiment_id,
                'symbol': selected_symbol,
                'config_sha256': digest,
                'reason': 'DUPLICATE_IN_AUTHORITATIVE_GLOBAL_MEMORY',
            })
            continue
        item = {
            'config': copy.deepcopy(candidate_config),
        ...
        if not _v278_exact_diversification_item(item):
            raise LabError('v2.0.78 exact diversification item rejected')
        if source_config is not None and not _v251_legal_frontier_item(
            source_config, item
        ):
            illegal.append({
                'experiment_id': experiment_id,
                'symbol': selected_symbol,
                'transition_axes': list(
                    _v251_transition_axes(source_config, candidate_config)
                ),
            })
            continue

        updated['novelty_frontier'] = [item]
        updated['v278_diversification_bridge'] = {
            **event,
            'status': 'EXACT_DIVERSIFICATION_BRIDGE_ADMITTED',
            'mode': 'V278_PRIORITY_SEALED_S1_ADMISSION',
            'experiment_id': experiment_id,
            'symbol': selected_symbol,
            'config_sha256': digest,
            'skipped_duplicates': skipped,
            'illegal_transitions': illegal,
            'skipped_used_experiment_ids': skipped_used,
        }
        return updated
    ...
    updated['v278_diversification_bridge'] = {
        **event,
        'status': (
            'DIVERSIFICATION_QUEUE_EXHAUSTED'
            if not illegal
            else 'BLOCKED_NO_LEGAL_SINGLE_AXIS_TRANSITION'
        ),
        'mode': 'V278_SEALED_DIVERSIFICATION_QUEUE_CLOSED',
        'experiment_id': None,
        'symbol': selected_symbol,
        'config_sha256': None,
        'skipped_duplicates': skipped,
...
def _v282_rotation_item(
    experiment_id: str,
    family_id: str,
    candidate_config: dict[str, Any],
) -> dict[str, Any]:
    """Build the exact controller-owned rotation row for one registered seed."""
    return {
        'config': copy.deepcopy(candidate_config),
        'v254_registration': {
            'version': V254_FRONTIER_SCOUT_VERSION,
        ...
        'v282_registry_rotation': {
            'version': V282_LEGAL_FRONTIER_RECOVERY_VERSION,
            'source': 'SEALED_KERNEL_REGISTRY_ROTATION',
            'experiment_id': experiment_id,
            'family_id': family_id,
            'selection_rule': (
                'FIRST_ELIGIBLE_IN_DETERMINISTIC_REGISTRY_ORDER'
            ),
            'schema_validated': True,
            'deduplicated_against_authoritative_global_memory': True,
...
def _v282_exact_rotation_item(item: Any) -> bool:
    """Recognize only an exact hash-bound v2.0.82 registry rotation seed."""
    if not isinstance(item, dict):
        return False
    config = item.get('config')
    rotation = item.get('v282_registry_rotation')
    if not isinstance(config, dict) or not isinstance(rotation, dict):
        return False
    experiment_id = config.get('experiment_id')
    family_id = config.get('family')
    if not isinstance(experiment_id, str) or not isinstance(family_id, str):
        return False
    if rotation != _v282_rotation_item(
        experiment_id, family_id, config
    )['v282_registry_rotation']:
        return False
    _, experiments = kernel.registry()
    experiment = experiments.get(experiment_id)
    if (
        not isinstance(experiment, dict)
        or experiment.get('family_id') != family_id
    ):
        return False
    return _v260_exact_controller_registered_seed(item)


def _v282_experiment_timeframe(experiment: dict[str, Any]) -> str | None:
    for key in ('effective_timeframe', 'timeframe'):
...
def _v282_registry_rotation_pool() -> tuple[tuple[str, str], ...]:
    """Order the sealed registry deterministically by family then experiment."""
    _, experiments = kernel.registry()
    ...
    for experiment_id, experiment in experiments.items():
        ...
        if not isinstance(family_id, str) or not family_id:
            continue
        rows.append((family_id, experiment_id))
...
def _v282_set_hash(
    ...
    """Hash a candidate set independently of scan order."""
    return _v254_canonical_hash(
        sorted([str(row.get(key) or '') for key in keys] for row in rows)
    )


def _v282_reason_distribution(
    rejected: list[dict[str, Any]],
) -> dict[str, int]:
    counts = {reason: 0 for reason in V282_REJECTION_REASONS}
    for row in rejected:
        reason = row.get('reason')
        if reason not in counts:
            raise LabError('v2.0.82 rotation rejection reason is unknown')
        counts[reason] += 1
    return counts


def _v282_memory_snapshot_hash(
    historical: set[str],
    used_experiment_ids: set[str],
    used_config_hashes: set[str],
) -> str:
    return _v254_canonical_hash({
        'historical_candidate_hashes': sorted(historical),
        'used_experiment_ids': sorted(used_experiment_ids),
        'used_config_hashes': sorted(used_config_hashes),
    })


def _v282_previous_decision_hash(
    run_dir: Any,
    round_number: Any,
) -> str | None:
    """Bind this decision to the most recent prior recovery decision."""
    if not isinstance(run_dir, Path) or not isinstance(round_number, int):
        return None
    best_round = -1
    best_hash: str | None = None
    for round_path in sorted(run_dir.glob('round-*')):
        artifact = round_path / V282_RECOVERY_ARTIFACT_FILENAME
        ...
        try:
            ...
            raise LabError(
                'v2.0.82 previous recovery decision is unreadable'
            ) from exc
        if not isinstance(payload, dict):
            raise LabError('v2.0.82 previous recovery decision is malformed')
        prior_round = payload.get('research_round')
        ...
        if (
            ...
            raise LabError('v2.0.82 previous recovery decision is incomplete')
        if best_round < prior_round < round_number:
            best_round = prior_round
            best_hash = digest
    return best_hash


def _v282_legal_frontier_recovery(
    context: dict[str, Any],
    actor: str,
    root: Path | None,
    *,
    ...
    """Rotate to the next eligible registered seed, or fail closed explicitly.

    Every sealed replenishment queue shipped before this release is a finite
    hard-coded seed list. Once authoritative global memory has consumed all of
    them the inherited chain can only rebuild an identical empty frontier, so
    each later run repeats the same unproductive round without any measurable
    state change. This lane is the single causal change of v2.0.82: when the
    frontier is still empty after every inherited lane, candidates are
    reconstructed from the sealed kernel registry in one deterministic order
    and at most one is admitted.

    Nothing is invented. Each row is rebuilt with the sealed kernel schema,
    hash-bound to its registry entry, deduplicated against both the bounded
    round context and authoritative global memory, and must still satisfy the
    inherited single-material-axis contract. Negative memory and quarantine
    identities reach this lane through the same inherited identity reader used
    by v2.0.54. When nothing eligible remains, an explicit fail-closed decision
    with reason codes and set hashes is recorded instead of a silent repeat.
    """
    frontier = context.get('novelty_frontier')
    if not isinstance(frontier, list) or frontier:
        return context
    source_config = _v251_source_config(context)
    ...
    if actor == 'claude':
        registered = updated.get('registered_candidate_contract')
        dual = (
            registered.get('dual_lane_contract')
            if isinstance(registered, dict)
            else None
        )
        if isinstance(dual, dict) and isinstance(
            dual.get('excluded_peer_family'), str
        ):
            excluded_family = dual['excluded_peer_family']
    if not isinstance(excluded_family, str) or not excluded_family:
        excluded_family = None
    ...
    _, experiments = kernel.registry()
    pool = _v282_registry_rotation_pool()

    eligible: list[dict[str, Any]] = []
    rejected: list[dict[str, Any]] = []
    selected_item: dict[str, Any] | None = None

    for family_id, experiment_id in pool:
        ...
        if family_id == excluded_family:
            rejected.append({
                ...
                'symbol': None,
                'config_sha256': None,
                'reason': 'EXCLUDED_PEER_FAMILY',
            })
            continue
        if _v282_experiment_timeframe(experiment) not in (
            V282_ELIGIBLE_TIMEFRAMES
        ):
            rejected.append({
                'experiment_id': experiment_id,
                'family_id': family_id,
        ...
        if not isinstance(universe, list) or not universe:
            raise LabError('v2.0.82 rotation registry universe is invalid')

        admitted: dict[str, Any] | None = None
        local_rejections: list[dict[str, Any]] = []
        for symbol in sorted(
            value for value in universe if isinstance(value, str)
        ):
            candidate_config = kernel.validate_config(
                kernel.performance_config(experiment, symbol)
            )
            ...
            if digest in used_config_hashes:
                local_rejections.append({
                    **row, 'reason': 'DUPLICATE_IN_ROUND_CONTEXT',
                })
                continue
            if digest in historical:
                ...
                continue
            item = _v282_rotation_item(
                experiment_id, family_id, candidate_config
            )
            if not _v282_exact_rotation_item(item):
                local_rejections.append({
                    **row, 'reason': 'NOT_EXACT_REGISTERED_SEED',
                })
                continue
            if not _v251_legal_frontier_item(source_config, item):
                local_rejections.append({
                    **row, 'reason': 'NO_LEGAL_SINGLE_AXIS_TRANSITION',
                })
                continue
            admitted = {'row': row, 'item': item}
            break

        # Every rejection is recorded even when a later symbol of the same
        # experiment is admitted; a discarded reason is what made the inherited
        # exhaustion states impossible to reconcile.
        rejected.extend(local_rejections)
        if admitted is None:
            continue
        eligible.append(admitted['row'])
        if selected_item is None:
            selected_item = admitted['item']

    selected = eligible[0] if eligible else None
    core = {
        'schema_version': V282_RECOVERY_SCHEMA_VERSION,
        'version': V282_LEGAL_FRONTIER_RECOVERY_VERSION,
        'mode': 'V282_SEALED_REGISTRY_ROTATION',
        'actor': actor,
        'run_id': run_id,
        'task_id': updated.get('task_id'),
        'research_round': updated.get('research_round'),
        'bounded_epoch_id': updated.get('round_id'),
        'trial_count': updated.get('trial_count'),
        'status': (
            V282_STATUS_ADMITTED if selected else V282_STATUS_EXHAUSTED
        ),
        'registry_experiment_count': len(pool),
        'registry_family_count': len({family for family, _ in pool}),
        'excluded_peer_family': excluded_family,
        'max_admissions': V282_MAX_REGISTRY_ADMISSIONS,
        'eligible_count': len(eligible),
        'eligible_set_sha256': _v282_set_hash(
            eligible, ('experiment_id', 'symbol', 'config_sha256')
        ),
        'rejected_count': len(rejected),
        'rejected_set_sha256': _v282_set_hash(
            rejected, ('experiment_id', 'symbol', 'reason')
        ),
        'rejection_reason_distribution': _v282_reason_distribution(rejected),
        'rejected_sample': rejected[:V282_REJECTED_SAMPLE_LIMIT],
        'rejected_sample_truncated': (
            len(rejected) > V282_REJECTED_SAMPLE_LIMIT
        ),
        'memory_snapshot_sha256': _v282_memory_snapshot_hash(
            historical, used_experiment_ids, used_config_hashes
        ),
        'authoritative_global_memory_read': root is not None,
        'family_cursor': selected.get('family_id') if selected else None,
        'frontier_cursor': (
            selected.get('experiment_id') if selected else None
        ),
        'selected_experiment_id': (
            selected.get('experiment_id') if selected else None
        ),
        'selected_family_id': selected.get('family_id') if selected else None,
        'selected_symbol': selected.get('symbol') if selected else None,
        'selected_config_sha256': (
            selected.get('config_sha256') if selected else None
        ),
        'previous_decision_sha256': previous_decision_sha256,
        'next_action': (
            'PROPOSE_ROTATED_REGISTERED_CANDIDATE'
            if selected
            else 'FAIL_CLOSED_CONTROLLER_REVIEW_REQUIRED'
        ),
        'selection_is_deterministic': True,
        'memory_quarantine_and_duplicate_filters_preserved': True,
        'single_material_axis_required': True,
        'model_generated_executable_code': False,
        'raw_proposal_executed': False,
        'provider_invoked_by_recovery': False,
        'new_families_auto_registered': False,
        's1_only': True,
        's2_s4_opened': False,
        'controller_only_promotion': True,
        'trading_actions': False,
...
def _v276_global_memory_packet_a_filter(
    context: dict[str, Any],
    actor: str,
    root: Path | None,
) -> dict[str, Any]:
    """Remove only an exact globally duplicated Packet-A before dispatch."""
    event = context.get('v261_packet_a_replenishment')
    if actor != 'codex' or not isinstance(event, dict):
        return context
    if (
        event.get('version') != V261_RSI_GATED_REVERSION_VERSION
        or event.get('mode') != 'V261_EXACT_REVIEWED_PACKET_A_ADMISSION'
        or event.get('actor') != 'codex'
        or event.get('experiment_id') != V261_PACKET_A_EXPERIMENT_ID
        or event.get('family_id') != 'RSI_GATED_REVERSION'
        or event.get('s1_only') is not True
        or event.get('controller_only_promotion') is not True
        or event.get('trading_actions') is not False
        or event.get('exchange_api_access') is not False
    ):
        raise LabError('v2.0.76 Packet-A filter provenance drifted')

    frontier = context.get('novelty_frontier')
    if (
        not isinstance(frontier, list)
        or len(frontier) != 1
        or not _v275_exact_packet_a_item(frontier[0])
    ):
        raise LabError('v2.0.76 exact Packet-A filter frontier is missing')
    config = frontier[0].get('config')
    digest = _v254_canonical_hash(config)
    if digest != event.get('config_sha256'):
        raise LabError('v2.0.76 Packet-A filter hash binding drifted')
    ...
    duplicate = digest in historical
    updated = copy.deepcopy(context)
    if duplicate:
        updated['novelty_frontier'] = []
    updated['v276_packet_a_global_memory_filter'] = {
        'version': V276_PACKET_A_GLOBAL_MEMORY_FILTER_VERSION,
        'actor': 'codex',
        'status': (
            'DUPLICATE_PACKET_A_FILTERED'
            if duplicate
            else 'NOVEL_PACKET_A_PRESERVED'
        ),
        'experiment_id': V261_PACKET_A_EXPERIMENT_ID,
        'config_sha256': digest,
        'historical_candidate_hash_count': len(historical),
        'duplicate_in_authoritative_global_memory': duplicate,
        'frontier_removed_before_provider': duplicate,
        'authoritative_duplicate_reader_reused': root is not None,
        'staging_fixture_memory_scan_bypassed': root is None,
        'provider_invoked_by_filter': False,
        'raw_proposal_executed': False,
...
def _v265_reviewed_proposal_registry() -> dict[str, tuple[str, ...]]:
    ...
    registries = (
        ...
        (
            HERE / 'research' / V278_REVIEWED_SEEDS_FILENAME,
            V278_REVIEWED_REGISTRY_ID,
            V278_SOURCE_PROPOSAL_SHA256,
        ),
    )
...
def _v277_select_sealed_implementation_packet(
    lifecycle: dict[str, Any] | None = None,
) -> dict[str, Any] | None:
    """Select one exact data-only packet for controller implementation review."""
    current = lifecycle or _v265_scout_inbox_lifecycle()
    if current.get('invalid_count') or current.get('raw_hard_limit_reached'):
        raise LabError('v2.0.77 implementation queue refuses unsafe inbox')

    ready = [
        row for row in current.get('records', [])
        if isinstance(row, dict)
        and row.get('state') == 'READY_FOR_SEALED_IMPLEMENTATION'
    ]
    if not ready:
        return None
    selected = sorted(
    ...
    digest = str(selected.get('proposal_sha256') or '')
    decision_digest = str(selected.get('producer_decision_sha256') or '')
    if (
        re.fullmatch(r'[0-9a-f]{64}', digest) is None
        or re.fullmatch(r'[0-9a-f]{64}', decision_digest) is None
        or not hypothesis_id
    ):
        raise LabError('v2.0.77 implementation queue identity is invalid')

    v267_decision = _v267_read_decision(digest)
    decision = v267_decision or _v266_read_decision(digest)
    if decision is None:
        raise LabError('v2.0.77 implementation queue decision is missing')
    proposal = _v266_load_raw_proposal(file_name, digest)
    family_id = decision.get('registered_family_id')
    matches = _v266_explicit_family_matches(proposal)
    families, _ = kernel.registry()
    if (
        _v254_canonical_hash(decision) != decision_digest
        or decision.get('status') != 'READY_FOR_SEALED_IMPLEMENTATION'
        or decision.get('source_file') != file_name
        or decision.get('source_hypothesis_id') != hypothesis_id
        or decision.get('source_proposal_sha256') != digest
        or not isinstance(family_id, str)
        or family_id not in families
        or matches != [family_id]
        or decision.get('registered_family_matches') != [family_id]
        or decision.get('candidate_baseline_negative_control_required') is not True
        or decision.get('contains_executable_code') is not False
        or decision.get('raw_proposal_executed') is not False
        or decision.get('automatically_registered') is not False
        or decision.get('sealed_registry_change_required') is not True
        or decision.get('controller_only_registration') is not True
        or decision.get('s1_only') is not True
        or decision.get('s2_s4_opened') is not False
        or decision.get('trading_actions') is not False
        or decision.get('exchange_api_access') is not False
    ):
        raise LabError('v2.0.77 implementation queue decision drifted')

    capabilities = decision.get('required_data_capabilities')
    if decision.get('version') == V267_DATA_CAPABILITY_VERSION:
        if (
            not isinstance(capabilities, list)
            or not capabilities
            or any(
                not isinstance(row, dict)
                or row.get('capability') not in {
                    'INSTALLED_RAW_OHLCV', 'DERIVABLE_FROM_OHLCV'
                }
                for row in capabilities
            )
            or decision.get('missing_external_data') != []
            or decision.get('ambiguous_data_requirements') != []
        ):
            raise LabError('v2.0.77 implementation queue data drifted')
    elif decision.get('version') == V266_FRONTIER_PRODUCER_VERSION:
        if (
            decision.get('missing_offline_data') != []
            or not set(decision.get('required_data') or ()).issubset(
                V266_OFFLINE_AVAILABLE_DATA
            )
        ):
            raise LabError('v2.0.77 legacy implementation data drifted')
        capabilities = []
    else:
        raise LabError('v2.0.77 implementation decision version drifted')

    if (
        decision.get('timeframes') != proposal.get('timeframes')
        or decision.get('bounded_parameters')
        != proposal.get('bounded_parameters')
        or decision.get('candidate_thesis') != proposal.get('family_thesis')
        or decision.get('baseline_thesis') != proposal.get('baseline_thesis')
        or decision.get('negative_control_thesis')
        != proposal.get('negative_control_thesis')
        or decision.get('falsification') != proposal.get('falsification')
    ):
        raise LabError('v2.0.77 implementation packet source drifted')

    packet = {
        'version': V277_SEALED_IMPLEMENTATION_QUEUE_VERSION,
        'status': 'SELECTED_FOR_CONTROLLER_IMPLEMENTATION_REVIEW',
        'selection_order': 'hypothesis_id,proposal_sha256,file',
        ...
        'registered_family_id': family_id,
        'required_data': copy.deepcopy(decision.get('required_data')),
        'required_data_capabilities': copy.deepcopy(capabilities),
        'timeframes': copy.deepcopy(proposal['timeframes']),
        'bounded_parameters': copy.deepcopy(proposal['bounded_parameters']),
        'candidate_thesis': proposal['family_thesis'],
        'baseline_thesis': proposal['baseline_thesis'],
        ...
        'falsification': copy.deepcopy(proposal['falsification']),
        'candidate_baseline_negative_control_required': True,
        'data_only_review_packet': True,
        'contains_executable_code': False,
        'raw_proposal_executed': False,
        'provider_invoked_by_selector': False,
        'automatically_registered': False,
        'sealed_registry_change_required': True,
        'controller_only_registration': True,
        's1_only': True,
        's1_invoked_by_selector': False,
        's2_s4_opened': False,
        'trading_actions': False,
        'exchange_api_access': False,
        'unknown_errors_fail_closed': True,
    }
    packet['packet_payload_sha256'] = _v254_canonical_hash(packet)
    return packet
...
_V280_SAFETY_GATE_KEYS = frozenset({
    'no_leakage',
    'data_integrity',
    'accounting_reconciled',
    'execution_model_compliant',
    'single_position_compliant',
    'costs_included',
    'funding_included',
    'deterministic_rerun',
    'baseline_beaten',
    'negative_control_beaten',
    'all_s1_gates',
})


def _v280_json_chars(value: Any) -> int:
...
def _v280_hash_json(value: Any) -> str:
    encoded = json.dumps(
        value,
        sort_keys=True,
        ensure_ascii=False,
...
def _v280_metrics(raw: Any) -> dict[str, Any]:
    source = raw if isinstance(raw, dict) else {}
    return {
        key: copy.deepcopy(source.get(key))
        for key in _V280_METRIC_KEYS
        if key in source
    }


def _v280_gate_contrast(raw: Any) -> dict[str, Any]:
    source = raw if isinstance(raw, dict) else {}
    ...
    for key in sorted(source):
        ...
        if (
            ...
            or key.endswith('_s1_only')
        ):
            compact[key] = copy.deepcopy(value)
    present_safety = [key for key in _V280_SAFETY_GATE_KEYS if key in source]
    compact['safety_contract_gates_all_true'] = all(
        source[key] is True
        for key in present_safety
        if key not in {
            'baseline_beaten',
            'negative_control_beaten',
            'all_s1_gates',
...
def _v280_contrast_metrics(raw: Any) -> dict[str, Any]:
    source = raw if isinstance(raw, dict) else {}
    return {
        key: copy.deepcopy(source.get(key))
        for key in _V280_CONTRAST_KEYS
        if key in source
    }


def _v280_worst_fold(raw: Any) -> dict[str, Any] | None:
    folds = raw if isinstance(raw, list) else []
    valid = [item for item in folds if isinstance(item, dict)]
    if not valid:
        return None
    ...
    def score(item: dict[str, Any]) -> tuple[float, str]:
        ...
        return expectancy, str(item.get('fold_id') or '')

    item = min(valid, key=score)
    metrics = item.get('metrics') if isinstance(item.get('metrics'), dict) else {}
    return {
        'fold_id': item.get('fold_id'),
        'metrics': {
            key: copy.deepcopy(metrics.get(key))
            for key in _V280_WORST_FOLD_KEYS
            if key in metrics
        },
    }


def _v280_compact_candidate(raw: Any) -> dict[str, Any] | None:
    if not isinstance(raw, dict) or not isinstance(raw.get('config'), dict):
        return None
    primary = raw.get('primary_change')
    if isinstance(primary, dict):
        compact_primary = {
            key: copy.deepcopy(primary.get(key))
            for key in ('component', 'from', 'to', 'atomic_bundle')
            if key in primary
        }
        if primary.get('rationale') is not None:
            compact_primary['rationale'] = str(primary['rationale'])[:180]
    else:
        compact_primary = None
    return {
        'candidate_id': raw.get('candidate_id'),
        'hypothesis_id': raw.get('hypothesis_id'),
        'config': copy.deepcopy(raw['config']),
        'primary_change': compact_primary,
    }


def _v280_compact_evidence(raw: Any) -> dict[str, Any] | None:
    if not isinstance(raw, dict):
        return None
    compact = {
        'candidate_id': raw.get('candidate_id'),
        'controller_verdict': raw.get('controller_verdict'),
        'metrics': _v280_metrics(raw.get('metrics')),
        'gates': _v280_gate_contrast(raw.get('gates')),
        'observations': copy.deepcopy(raw.get('observations', []))[:4]
        if isinstance(raw.get('observations'), list) else [],
        'delta_vs_baseline': _v280_contrast_metrics(
            raw.get('delta_vs_baseline')
        ),
        'delta_vs_negative_control': _v280_contrast_metrics(
            raw.get('delta_vs_negative_control')
        ),
    }
    identity = raw.get('identity')
    strategy_config_sha256 = (
        identity.get('strategy_config_sha256')
        if isinstance(identity, dict)
        else raw.get('strategy_config_sha256')
    )
...
def _v281_full_s1_counterexamples(
    compact_candidates: list[dict[str, Any]],
    compact_evidence: list[dict[str, Any]],
) -> tuple[list[dict[str, Any]], dict[str, str]]:
    """Read one exact current-run evidence artifact; never scan history."""
    candidate_ids = [str(row.get('candidate_id') or '') for row in compact_candidates]
    match_rows = [
        re.fullmatch(
            r'(?:codex|claude)-(\d{8}T\d{6}Z)-r(\d{2})-c\d{2}',
            candidate_id,
        )
        for candidate_id in candidate_ids
    ]
    if not candidate_ids or any(match is None for match in match_rows):
        raise LabError('v2.0.81 post-S1 candidate run identity is invalid')
    stamps = {match.group(1) for match in match_rows if match is not None}
    rounds = {int(match.group(2)) for match in match_rows if match is not None}
    if len(stamps) != 1 or len(rounds) != 1:
        raise LabError('v2.0.81 post-S1 candidate run identity drifted')
    stamp = next(iter(stamps))
    round_number = next(iter(rounds))
    expected_run_id = f'tdh-strategy-lab-v2-{stamp}'
    evidence_path = (
        V281_RUNS_ROOT
        / expected_run_id
        / f'round-{round_number:02d}'
        / V281_FINANCIAL_EVIDENCE_FILENAME
    )
    if not evidence_path.is_file() or evidence_path.is_symlink():
        raise LabError('v2.0.81 immutable S1 evidence artifact is missing')
    try:
        full = json.loads(evidence_path.read_text(encoding='utf-8'))
    except (OSError, UnicodeError, json.JSONDecodeError) as exc:
        raise LabError('v2.0.81 immutable S1 evidence artifact is invalid') from exc
    if not isinstance(full, dict):
        raise LabError('v2.0.81 immutable S1 evidence shape is invalid')
    if full.get('source_run_id') != expected_run_id:
        raise LabError('v2.0.81 immutable S1 run identity drifted')
    if full.get('source_round') != round_number:
        raise LabError('v2.0.81 immutable S1 round identity drifted')
    source_result_sha256 = str(full.get('source_result_sha256') or '')
    if re.fullmatch(r'[0-9a-f]{64}', source_result_sha256) is None:
        raise LabError('v2.0.81 immutable S1 result hash is invalid')

    full_rows = full.get('candidates')
    if not isinstance(full_rows, list):
        raise LabError('v2.0.81 immutable S1 candidates are missing')
    full_by_id: dict[str, dict[str, Any]] = {}
    for row in full_rows:
        if not isinstance(row, dict):
            continue
        candidate_id = str(row.get('candidate_id') or '')
        if not candidate_id or candidate_id in full_by_id:
            raise LabError('v2.0.81 immutable S1 candidate identity is invalid')
        full_by_id[candidate_id] = row

    evidence_ids = [str(row.get('candidate_id') or '') for row in compact_evidence]
    if candidate_ids != evidence_ids:
        raise LabError('v2.0.81 prompt/full S1 identity alignment drifted')

    counterexamples = []
    provenance: dict[str, str] = {}
    for prompt_candidate in compact_candidates:
        candidate_id = str(prompt_candidate.get('candidate_id') or '')
        source = full_by_id.get(candidate_id)
        if source is None:
            raise LabError('v2.0.81 prompt candidate is absent from full S1 evidence')
        source_config = source.get('strategy_config')
        prompt_config = prompt_candidate.get('config')
        if not isinstance(source_config, dict) or source_config != prompt_config:
            raise LabError('v2.0.81 prompt/full S1 strategy config drifted')
        expected_config_sha256 = _v280_hash_json(source_config)
        recorded_config_sha256 = str(source.get('strategy_config_sha256') or '')
        if (
            re.fullmatch(r'[0-9a-f]{64}', recorded_config_sha256) is None
            or recorded_config_sha256 != expected_config_sha256
        ):
            raise LabError('v2.0.81 immutable S1 strategy hash drifted')
        worst = _v280_worst_fold(source.get('fold_results'))
        if worst is None:
            raise LabError('v2.0.81 immutable S1 fold counterexample is missing')
        provenance[candidate_id] = recorded_config_sha256
        counterexamples.append({
            'candidate_id': candidate_id,
            **worst,
            'evidence_source': 'immutable_s1_financial_evidence',
            'source_result_sha256': source_result_sha256,
        })
    return counterexamples, provenance


def _v280_compact_positive_memory(raw: Any) -> dict[str, Any]:
    source = raw if isinstance(raw, dict) else {}
    rows = source.get('top_verified_current_positives')
    compact_rows = []
    for row in rows[:2] if isinstance(rows, list) else []:
        ...
        compact_rows.append({
            ...
            for key in (
                'positive_class', 'experiment_id', 'family', 'symbol',
                'timeframe', 'strategy_config_sha256', 'controller_verdict',
            )
            if key in row
        } | {'metrics': _v280_metrics(row.get('metrics'))})
    return {
        'memory_version': source.get('memory_version'),
        'verified_current_positive_count': source.get(
            'verified_current_positive_count'
        ),
        'legacy_positive_quarantined_count': source.get(
            'legacy_positive_quarantined_count'
        ),
        'top_verified_current_positives': compact_rows,
        'interpretation_contract': copy.deepcopy(
            source.get('interpretation_contract', {})
        ),
    }


def _v280_compact_prior_memory(raw: Any) -> dict[str, Any]:
    source = raw if isinstance(raw, dict) else {}
    verified = source.get('verified_s1')
    compact_verified = []
    for row in verified[-2:] if isinstance(verified, list) else []:
        if not isinstance(row, dict):
            continue
        compact_verified.append({
            key: copy.deepcopy(row.get(key))
            for key in (
                'candidate_id', 'controller_verdict', 'family',
                'experiment_id', 'timeframe', 'strategy_config_sha256',
            )
            if key in row
        } | {'metrics': _v280_metrics(row.get('metrics'))})

    def findings(name: str) -> list[dict[str, Any]]:
    ...
    synthesis = source.get('controller_synthesis')
    compact_synthesis = {}
    if isinstance(synthesis, dict):
        compact_synthesis = {
            's1_pass_ids': copy.deepcopy(synthesis.get('s1_pass_ids', []))[:8],
            'consensus_ids': copy.deepcopy(synthesis.get('consensus_ids', []))[:8],
            'next_selection_rule_sha256': hashlib.sha256(str(
                synthesis.get('next_selection_rule', '')
            ).encode('utf-8')).hexdigest(),
        }
    result = {
        'source_run_id': source.get('source_run_id'),
        'source_round': source.get('source_round'),
    }
    codex = findings('codex_findings')
    claude = findings('claude_findings')
    if codex:
        result['codex_findings'] = codex
    if claude:
        result['claude_findings'] = claude
    if compact_synthesis:
        result['controller_synthesis'] = compact_synthesis
    if compact_verified:
        result['verified_s1'] = compact_verified
    return result


def v280_compact_post_s1_headroom_packet(raw: Any) -> dict[str, Any]:
    """Build the bounded prompt fallback; never rewrite raw evidence on disk."""
    if not isinstance(raw, dict):
        raise LabError('v2.0.80 post-S1 analysis packet is not an object')
    candidates = raw.get('candidates')
    evidence = raw.get('s1_evidence')
    if not isinstance(candidates, list) or not isinstance(evidence, list):
        raise LabError('v2.0.80 post-S1 candidate evidence is missing')
    if len(candidates) != len(evidence) or not 1 <= len(candidates) <= 4:
        raise LabError('v2.0.80 post-S1 candidate count is outside bound')

    compact_candidates = [_v280_compact_candidate(row) for row in candidates]
    compact_evidence = [_v280_compact_evidence(row) for row in evidence]
    if any(row is None for row in compact_candidates + compact_evidence):
        raise LabError('v2.0.80 post-S1 candidate shape is invalid')
    candidate_ids = [row['candidate_id'] for row in compact_candidates]
    evidence_ids = [row['candidate_id'] for row in compact_evidence]
    if candidate_ids != evidence_ids or len(set(candidate_ids)) != len(candidate_ids):
        raise LabError('v2.0.80 post-S1 candidate identity alignment drifted')

    counterexamples = []
    for row in evidence:
        worst = _v280_worst_fold(row.get('fold_results'))
        if worst is not None:
            counterexamples.append({
                'candidate_id': row.get('candidate_id'),
                **worst,
                'evidence_source': 'inline_full_fold_results',
            })
    if not counterexamples:
        counterexamples, provenance = _v281_full_s1_counterexamples(
            compact_candidates,
            compact_evidence,
        )
        for row in compact_evidence:
            ...
            expected_sha256 = provenance[candidate_id]
            if isinstance(recorded, dict):
                current_sha256 = recorded.get('strategy_config_sha256')
                if current_sha256 not in {None, expected_sha256}:
                    raise LabError('v2.0.81 prompt S1 provenance drifted')
            row['provenance'] = {
                'strategy_config_sha256': expected_sha256,
            }

    def counterexample_score(item: dict[str, Any]) -> tuple[float, str]:
        metrics = item.get('metrics') if isinstance(item.get('metrics'), dict) else {}
        try:
            expectancy = float(metrics.get('expectancy_r'))
        except (TypeError, ValueError):
            expectancy = float('inf')
        return expectancy, str(item.get('candidate_id') or '')

    strongest_counterexample = min(counterexamples, key=counterexample_score)

    primary_changes = [
        row.get('primary_change') for row in compact_candidates
    ]
    shared_primary_change = None
    if primary_changes and all(
        change == primary_changes[0] for change in primary_changes
    ):
        shared_primary_change = copy.deepcopy(primary_changes[0])
        for row in compact_candidates:
            row.pop('primary_change', None)

    batch = raw.get('controller_batch')
    batch_source = batch if isinstance(batch, dict) else {}
    compact_batch = {
        key: copy.deepcopy(value)
        for key, value in sorted(batch_source.items())
        if isinstance(value, (str, int, float, bool))
        and key not in {'audit_instruction'}
    }
    compact_batch.update({
        'full_raw_evidence_remains_on_vps': True,
        'positive_pnl_memory': _v280_compact_positive_memory(
            batch_source.get('positive_pnl_memory')
        ),
        'prior_shared_research_context': _v280_compact_prior_memory(
            batch_source.get('prior_shared_research_context')
        ),
    })
    compact = {
        'contract_version': raw.get('contract_version'),
        'research_round': raw.get('research_round'),
        'verdict': raw.get('verdict'),
        'candidates': compact_candidates,
        's1_evidence': compact_evidence,
        'shared_primary_change': shared_primary_change,
        'strongest_counterexample': strongest_counterexample,
        'controller_batch': compact_batch,
        'v280_headroom_contract': {
            'version': V280_POST_S1_HEADROOM_VERSION,
            'raw_packet_sha256': _v280_hash_json(raw),
            'raw_evidence_remains_on_vps': True,
            'preserved': [
                'all_current_configs',
                'hard_metrics',
                'failed_gates',
    ...
    size = _v280_json_chars(compact)
    if size > V280_POST_S1_ANALYSIS_MAX_CHARS:
        # Narratives are advisory; remove them before touching any config,
        # financial metric, failed gate, fold counterexample or provenance.
        for row in compact['candidates']:
            primary = row.get('primary_change')
            if isinstance(primary, dict):
                primary.pop('rationale', None)
        shared_primary = compact.get('shared_primary_change')
        if isinstance(shared_primary, dict):
            shared_primary.pop('rationale', None)
        memory = compact['controller_batch']['prior_shared_research_context']
        memory.pop('codex_findings', None)
        memory.pop('claude_findings', None)
        size = _v280_json_chars(compact)
    if size > V280_POST_S1_ANALYSIS_MAX_CHARS:
        raise LabError(
            'v2.0.80 post-S1 headroom fallback cannot preserve evidence: '
            + str(size)
        )
    return compact


V280_BASE_POST_S1_COMPACTOR = compact_post_s1_precheck_packet
V280_POST_S1_OWNER = sys.modules.get(
    getattr(V280_BASE_POST_S1_COMPACTOR, '__module__', '')
)


def compact_post_s1_precheck_packet(raw: Any):
    """Keep the inherited result unless the one observed v2.0.41 edge occurs."""
    try:
        return V280_BASE_POST_S1_COMPACTOR(raw)
    except LabError as exc:
        message = str(exc)
        if re.fullmatch(
            re.escape(V280_LEGACY_HEADROOM_ERROR_PREFIX) + r'\s*[0-9]+',
            message,
        ) is None:
            raise
        compact = v280_compact_post_s1_headroom_packet(raw)
        return compact, {
            'version': V280_POST_S1_HEADROOM_VERSION,
            'compaction_level': 3,
            'legacy_error': message,
            'input_chars': _v280_json_chars(raw),
            'output_chars': _v280_json_chars(compact),
            'analysis_max_chars': V280_POST_S1_ANALYSIS_MAX_CHARS,
            'limits_unchanged': True,
            'raw_evidence_remains_on_vps': True,
            's1_gates_unchanged': True,
            'provider_invoked_by_compactor': False,
            'trading_actions': False,
            'exchange_api_access': False,
        }


def _bind_v280_post_s1_compactor() -> tuple[str, ...]:
    if V280_POST_S1_OWNER is None:
        raise RuntimeError('v2.0.80 cannot locate v2.0.41 compactor owner')
    targets = [
        V280_POST_S1_OWNER,
        v247,
        v246,
    ...
    for module in targets:
        current = getattr(module, 'compact_post_s1_precheck_packet', None)
        if current is V280_BASE_POST_S1_COMPACTOR:
            module.compact_post_s1_precheck_packet = (
                compact_post_s1_precheck_packet
            )
            bound.append(module.__name__)
        elif current is compact_post_s1_precheck_packet:
            bound.append(module.__name__)
    if getattr(
        V280_POST_S1_OWNER,
        'compact_post_s1_precheck_packet',
        None,
    ) is not compact_post_s1_precheck_packet:
        raise RuntimeError('v2.0.80 v2.0.41 compactor binding failed closed')
...
class Controller(V246_DISPATCH_BASE):
    ...
    def _v275_cache_packet_a_example_frontier(
        self,
        context: dict[str, Any],
        round_number: int,
        actor: str,
    ) -> None:
        """Cache one exact Packet-A row for the inherited example builder."""
        self._v275_packet_a_example_frontier = None
        event = context.get('v261_packet_a_replenishment')
        if actor != 'codex' or not isinstance(event, dict):
            return

        packet_filter = context.get('v276_packet_a_global_memory_filter')
        if isinstance(packet_filter, dict):
            duplicate = packet_filter.get(
                'duplicate_in_authoritative_global_memory'
            )
            if (
                packet_filter.get('version')
                != V276_PACKET_A_GLOBAL_MEMORY_FILTER_VERSION
                or packet_filter.get('experiment_id')
                != V261_PACKET_A_EXPERIMENT_ID
                or packet_filter.get('config_sha256')
                != event.get('config_sha256')
                or duplicate not in {True, False}
                or packet_filter.get('provider_invoked_by_filter') is not False
                or packet_filter.get('trading_actions') is not False
                or packet_filter.get('exchange_api_access') is not False
            ):
                raise LabError('v2.0.76 Packet-A filter audit drifted')
            if duplicate:
                if (
                    packet_filter.get('status')
                    != 'DUPLICATE_PACKET_A_FILTERED'
                    or packet_filter.get('frontier_removed_before_provider')
                    is not True
                    or context.get('novelty_frontier') != []
                ):
                    raise LabError('v2.0.76 filtered Packet-A state drifted')
                return

        queue_event = context.get('v269_reviewed_seed_replenishment')
        filter_event = context.get('v274_global_memory_queue_filter')
        if (
            not isinstance(queue_event, dict)
            or queue_event.get('status') != 'REVIEWED_SEED_QUEUE_EXHAUSTED'
            or not isinstance(filter_event, dict)
            or filter_event.get('version')
            != V274_GLOBAL_MEMORY_QUEUE_FILTER_VERSION
            or filter_event.get('selected_experiment_id') is not None
            or filter_event.get('authoritative_duplicate_reader_reused')
            is not True
            or event.get('version') != V261_RSI_GATED_REVERSION_VERSION
            or event.get('mode') != 'V261_EXACT_REVIEWED_PACKET_A_ADMISSION'
            or event.get('actor') != 'codex'
            or event.get('experiment_id') != V261_PACKET_A_EXPERIMENT_ID
            or event.get('family_id') != 'RSI_GATED_REVERSION'
            ...
            raise LabError('v2.0.75 Packet-A example provenance drifted')
        ...
        if (
            ...
            raise LabError('v2.0.75 exact Packet-A example frontier is missing')
        config = frontier[0].get('config')
        digest = _v254_canonical_hash(config)
        if digest != event.get('config_sha256'):
            raise LabError('v2.0.75 Packet-A example hash binding drifted')

        source_config = _v251_source_config(context)
        if (
            not isinstance(source_config, dict)
            or not _v260_exact_controller_registered_seed({
                'config': source_config,
            })
            or source_config.get('symbol') != config.get('symbol')
            or _v251_transition_axes(source_config, config) != ('family',)
        ):
            raise LabError('v2.0.75 Packet-A source transition is not exact')

        self._v275_packet_a_example_frontier = {
            'version': V275_PACKET_A_EXAMPLE_BRIDGE_VERSION,
            'actor': actor,
            'research_round': round_number,
            'experiment_id': V261_PACKET_A_EXPERIMENT_ID,
            'config_sha256': digest,
            ...
            'transition_axes': ['family'],
            'frontier': copy.deepcopy(frontier),
            'example_only': True,
            'packet_a_replenishment_reused': True,
            'controller_only_promotion': True,
            'raw_proposal_executed': False,
            'trading_actions': False,
            'exchange_api_access': False,
    ...
    def _v275_validated_packet_a_example_frontier(
        self,
    ) -> list[dict[str, Any]]:
        record = getattr(self, '_v275_packet_a_example_frontier', None)
        if not isinstance(record, dict):
            raise LabError('v2.0.75 Packet-A example cache is missing')
        frontier = record.get('frontier')
        if (
            record.get('version') != V275_PACKET_A_EXAMPLE_BRIDGE_VERSION
            or record.get('selected_approach') != 'CHANGE_STRATEGY_FAMILY'
            or record.get('transition_axes') != ['family']
            or record.get('example_only') is not True
            or record.get('packet_a_replenishment_reused') is not True
            or record.get('trading_actions') is not False
            or record.get('exchange_api_access') is not False
            or not isinstance(frontier, list)
            or len(frontier) != 1
            or not _v275_exact_packet_a_item(frontier[0])
        ):
            raise LabError('v2.0.75 Packet-A example cache identity drifted')
        ...
        if (
            ...
            raise LabError('v2.0.75 Packet-A example transition drifted')

        temporary = copy.deepcopy(frontier)
        temporary[0]['selected_approach'] = record['selected_approach']
        temporary[0]['sha256_prefix'] = record['config_sha256'][:16]
        if 'selected_approach' in frontier[0] or 'sha256_prefix' in frontier[0]:
            raise LabError('v2.0.75 cached Packet-A frontier was mutated')
        if _v254_canonical_hash(temporary[0]['config']) != record['config_sha256']:
            raise LabError('v2.0.75 temporary Packet-A config hash drifted')
        return temporary
    ...
    def _diverse_frontier(self, *args: Any, **kwargs: Any) -> list[Any]:
        ...
        """
        if bool(getattr(self, '_v275_example_building', False)):
            return self._v275_validated_packet_a_example_frontier()
    ...
    def _v256_scout_on_frontier_exhaustion(
        ...
        if producer_decision is not None:
            inbox = _v261_scout_inbox_status()
        implementation_packet = _v277_select_sealed_implementation_packet(
            inbox
        )
        if implementation_packet is not None:
            atomic_json(
                round_dir / 'SEALED_IMPLEMENTATION_QUEUE_V277.json',
        ...
        if inbox['provider_allowed'] is not True:
            ...
            dispatch = {
                **base,
                ...
                'sealed_implementation_packet': implementation_packet,
    ...
    def round_context(self, round_number: int) -> dict[str, Any]:
        ...
        context = _v278_sealed_diversification_replenishment(
            context,
            actor,
            _v274_controller_memory_root(self),
        )
        v278_event = context.get('v278_diversification_bridge')
        if isinstance(v278_event, dict):
            round_dir = self.run_dir / f'round-{round_number:02d}'
            round_dir.mkdir(parents=True, exist_ok=True)
            atomic_json(
                round_dir / 'SEALED_DIVERSIFICATION_BRIDGE_V278.json',
                v278_event,
            )
        v278_status = (
            v278_event.get('status')
            if isinstance(v278_event, dict)
            else None
        )
        # An admitted v2.0.78 row must remain the only causal treatment in this
        # round. A structurally illegal v2.0.78 transition also stays closed;
        # the generic registry and Packet-A lanes may not silently bypass it.
        # Only a deterministically exhausted diversification queue may resume
        # the inherited fallback order.
        if v278_status not in {
            'EXACT_DIVERSIFICATION_BRIDGE_ADMITTED',
            'BLOCKED_NO_LEGAL_SINGLE_AXIS_TRANSITION',
        }:
            ...
            context = _v276_global_memory_packet_a_filter(
                context,
                actor,
                _v274_controller_memory_root(self),
            )
        v276_event = context.get('v276_packet_a_global_memory_filter')
        if isinstance(v276_event, dict):
            round_dir = self.run_dir / f'round-{round_number:02d}'
            round_dir.mkdir(parents=True, exist_ok=True)
            atomic_json(
                round_dir / 'PACKET_A_GLOBAL_MEMORY_FILTER_V276.json',
                v276_event,
            )
        # Every inherited lane above draws from a finite sealed seed list. When
        # all of them are consumed the frontier is still empty here, which is
        # the exact state that used to repeat unchanged for every later run.
        context = _v282_legal_frontier_recovery(
            context,
            actor,
            _v274_controller_memory_root(self),
            run_id=str(getattr(self, 'run_id', '')) or None,
            previous_decision_sha256=_v282_previous_decision_hash(
                getattr(self, 'run_dir', None),
                round_number,
            ),
            excluded_family=getattr(self, '_v225_codex_family', None),
        )
        v282_event = context.get('v282_legal_frontier_recovery')
        if isinstance(v282_event, dict):
            round_dir = self.run_dir / f'round-{round_number:02d}'
            round_dir.mkdir(parents=True, exist_ok=True)
            atomic_json(
                round_dir / V282_RECOVERY_ARTIFACT_FILENAME,
                v282_event,
            )
        self._v272_cache_example_frontier(context, round_number, actor)
        self._v275_cache_packet_a_example_frontier(
            context,
            round_number,
            actor,
        )
    ...
    def proposal_output_example(self, round_number: int, actor: str) -> str:
        packet_record = getattr(
            self,
            '_v275_packet_a_example_frontier',
            None,
        )
        if isinstance(packet_record, dict):
            if (
                actor != 'codex'
                or packet_record.get('actor') != actor
                or packet_record.get('research_round') != round_number
            ):
                raise LabError('v2.0.75 Packet-A example scope drifted')

            self._v275_example_building = True
            try:
                result = super().proposal_output_example(
                    round_number,
                    actor,
                )
            finally:
                self._v275_example_building = False

            audit = {
                key: copy.deepcopy(value)
                for key, value in packet_record.items()
                if key not in {'frontier', 'source_config'}
            }
            audit.update({
                'mode': 'V275_EXACT_PACKET_A_FOR_OUTPUT_EXAMPLE',
                'example_frontier_used': True,
                'historical_frontier_not_recomputed_for_example': True,
                'sealed_v228_change_family_shape_satisfied': True,
                'cached_frontier_unchanged': True,
                'provider_invoked_by_bridge': False,
                'proposal_validation_unchanged': True,
                's1_gates_unchanged': True,
            ...
            round_dir.mkdir(parents=True, exist_ok=True)
            atomic_json(
                round_dir / 'PACKET_A_EXAMPLE_FRONTIER_V275.json',
                audit,
            )
            return result
    ...
    def _v255_maybe_run_frontier_scout(
        ...
        if implementation_packet is not None:
            atomic_json(
                rd / 'SEALED_IMPLEMENTATION_QUEUE_V277.json',
                implementation_packet,
            )
        if inbox['provider_allowed'] is not True:
            blocking_reason = str(
...
V280_POST_S1_COMPACTOR_BINDINGS = _bind_v280_post_s1_compactor()
_bind_local_adapter()
...
def runtime_binding_contract() -> dict[str, Any]:
    ...
    return {
        ...
        'v275_exact_packet_a_replenishment_only': True,
        'v275_sealed_change_family_shape_only': True,
        'v275_historical_frontier_not_recomputed_for_example': True,
        'v275_cached_frontier_unchanged': True,
        'v275_provider_invoked_by_bridge': False,
        'v275_proposal_validation_unchanged': True,
        'v275_s1_gates_unchanged': True,
        'v275_unknown_errors_fail_closed': True,
        'v276_packet_a_global_memory_filter_version': (
            V276_PACKET_A_GLOBAL_MEMORY_FILTER_VERSION
        ),
        'v276_authoritative_full_history_duplicate_reader_reused': True,
        'v276_exact_packet_a_only': True,
        'v276_duplicate_removed_before_provider': True,
        'v276_novel_packet_a_preserved': True,
        'v276_provider_invoked_by_filter': False,
        'v276_proposal_validation_unchanged': True,
        'v276_s1_gates_unchanged': True,
        'v276_unknown_errors_fail_closed': True,
        'v277_sealed_implementation_queue_version': (
            V277_SEALED_IMPLEMENTATION_QUEUE_VERSION
        ),
        'v277_one_deterministic_packet_selected': True,
        'v277_proposal_and_decision_hash_bound': True,
        'v277_registered_family_and_data_capability_bound': True,
        'v277_data_only_review_packet': True,
        'v277_provider_invoked_by_selector': False,
        'v277_automatic_registration': False,
        'v277_s1_invoked_by_selector': False,
        'v277_unknown_errors_fail_closed': True,
        'v278_diversification_bridge_version': (
            V278_DIVERSIFICATION_BRIDGE_VERSION
        ),
        'v278_source_packet_hash_bound': True,
        'v278_exact_four_coin_universe': True,
        'v278_single_primary_change_universe_only': True,
        'v278_candidate_baseline_negative_control_bound': True,
        'v278_deferred_parameter_axes_remain_closed': True,
        'v278_authoritative_global_memory_checked': True,
        'v278_raw_proposal_never_executes': True,
        'v278_provider_invoked_by_bridge': False,
        'v278_s1_only': True,
        'v278_unknown_errors_fail_closed': True,
        'v280_post_s1_headroom_version': V280_POST_S1_HEADROOM_VERSION,
        'v280_exact_legacy_error_only': True,
        'v280_analysis_max_chars': V280_POST_S1_ANALYSIS_MAX_CHARS,
        'v280_raw_evidence_remains_on_vps': True,
        'v280_all_current_candidate_configs_preserved': True,
        'v280_hard_metrics_and_failed_gates_preserved': True,
        'v280_strongest_counterexample_preserved': True,
        'v280_negative_memory_and_provenance_bounded': True,
        'v280_compactor_owner_bound': getattr(
            V280_POST_S1_OWNER,
            'compact_post_s1_precheck_packet',
            None,
        ) is compact_post_s1_precheck_packet,
        'v280_provider_invoked_by_compactor': False,
        'v280_s1_gates_unchanged': True,
        'v280_unknown_errors_fail_closed': True,
        'v281_post_s1_fold_bridge_version': (
            V281_POST_S1_FOLD_BRIDGE_VERSION
        ),
        'v281_exact_runtime_artifact_lookup': True,
        'v281_candidate_run_round_path_bound': True,
        'v281_worst_fold_from_full_evidence': True,
        'v281_source_result_hash_bound': True,
        'v281_strategy_config_hash_bound': True,
        'v281_raw_folds_remain_on_vps': True,
        'v281_provider_invoked_by_artifact_bridge': False,
        'v281_s1_gates_unchanged': True,
        'v281_unknown_shapes_fail_closed': True,
        'v282_legal_frontier_recovery_version': (
            V282_LEGAL_FRONTIER_RECOVERY_VERSION
        ),
        'v282_recovery_schema_version': V282_RECOVERY_SCHEMA_VERSION,
        'v282_registry_rotation_is_deterministic': True,
        'v282_only_exact_registered_kernel_seeds': True,
        'v282_registry_rotation_hash_bound': True,
        'v282_single_material_axis_required': True,
        'v282_authoritative_global_memory_checked': True,
        ...
        'v282_exhausted_registry_fails_closed': True,
        'v282_new_families_auto_registered': False,
        'v282_model_generated_executable_code': False,
        'v282_provider_invoked_by_recovery': False,
        'v282_s1_only': True,
        'v282_s1_gates_unchanged': True,
        'v282_s2_s4_opened': False,
        'v282_unknown_errors_fail_closed': True,
...
def main(argv: list[str] | None = None) -> int:
    ...
    if contract['v277_proposal_and_decision_hash_bound'] is not True:
        raise RuntimeError('v2.0.77 implementation packet binding drifted')
    if contract['v277_automatic_registration'] is not False:
        raise RuntimeError('v2.0.77 automatic registration boundary drifted')
    if contract['v278_source_packet_hash_bound'] is not True:
        raise RuntimeError('v2.0.78 source packet binding drifted')
    if contract['v278_single_primary_change_universe_only'] is not True:
        raise RuntimeError('v2.0.78 single-axis implementation drifted')
    if contract['v278_provider_invoked_by_bridge'] is not False:
        raise RuntimeError('v2.0.78 provider boundary drifted')
    if contract['v280_compactor_owner_bound'] is not True:
        raise RuntimeError('v2.0.80 post-S1 compactor binding drifted')
    if contract['v280_s1_gates_unchanged'] is not True:
        raise RuntimeError('v2.0.80 S1 gate boundary drifted')
    if contract['v280_provider_invoked_by_compactor'] is not False:
        raise RuntimeError('v2.0.80 provider boundary drifted')
    if contract['v281_exact_runtime_artifact_lookup'] is not True:
        raise RuntimeError('v2.0.81 evidence artifact lookup drifted')
    if contract['v281_source_result_hash_bound'] is not True:
        raise RuntimeError('v2.0.81 result hash boundary drifted')
    if contract['v281_strategy_config_hash_bound'] is not True:
        raise RuntimeError('v2.0.81 strategy hash boundary drifted')
    if contract['v281_provider_invoked_by_artifact_bridge'] is not False:
        raise RuntimeError('v2.0.81 provider boundary drifted')
    if contract['v281_s1_gates_unchanged'] is not True:
        raise RuntimeError('v2.0.81 S1 gate boundary drifted')
    if contract['v282_only_exact_registered_kernel_seeds'] is not True:
        raise RuntimeError('v2.0.82 registry rotation source drifted')
    if contract['v282_registry_rotation_hash_bound'] is not True:
        raise RuntimeError('v2.0.82 rotation hash binding drifted')
    if contract['v282_single_material_axis_required'] is not True:
        raise RuntimeError('v2.0.82 single-axis boundary drifted')
    if contract['v282_authoritative_global_memory_checked'] is not True:
        raise RuntimeError('v2.0.82 global memory boundary drifted')
    if contract['v282_negative_memory_and_quarantine_preserved'] is not True:
        raise RuntimeError('v2.0.82 negative memory boundary drifted')
    if contract['v282_exhausted_registry_fails_closed'] is not True:
        raise RuntimeError('v2.0.82 exhaustion boundary drifted')
    if contract['v282_provider_invoked_by_recovery'] is not False:
        raise RuntimeError('v2.0.82 provider boundary drifted')
    if contract['v282_new_families_auto_registered'] is not False:
        raise RuntimeError('v2.0.82 family registration boundary drifted')
    if contract['v282_s1_gates_unchanged'] is not True:
        raise RuntimeError('v2.0.82 S1 gate boundary drifted')
    if contract['v282_s2_s4_opened'] is not False:
        raise RuntimeError('v2.0.82 staged research boundary drifted')
