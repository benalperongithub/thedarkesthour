...
SRC="$BASE/v2.0.69"
DST="$BASE/staging/v2.0.70"
TMP="$BASE/staging/.v2.0.70-build-$$"
REPO="/home/tdw/the-darkest-hour"
REPO_SOURCE="$REPO/controller/staging/v2.0.70"
GATE="/usr/local/sbin/tdh-lab-admin-gate"
PYTHON="/srv/tdh-research/phoenix-venv/bin/python"

EXPECTED_CONTROLLER_SHA256="2bb4521dee3d5e51702cc0c296fbad2d9a2cebc2f17fb686e566cc906730d6a3"
EXPECTED_TEST_SHA256="f1cac6940a30c6e6fbf51b2ed51cc860207ea1feb7732d750803975a72f14ce7"

cleanup() {
    if [[ -d "$TMP" && "$TMP" == "$BASE/staging/.v2.0.70-build-"* ]]; then
        rm -rf --one-file-system "$TMP"
    fi
}
trap cleanup EXIT

echo "===== 1. STOP AND BLOCK v2.0.69 DURING UPGRADE ====="
systemctl mask --runtime --now "$SERVICE" || true
sleep 1
test "$(systemctl is-active "$SERVICE" || true)" != "active"

echo "===== 2. VERIFY SEALED v2.0.69 ====="
test -d "$SRC"
...
"$GATE" preflight v2.0.69 >/tmp/tdh-v2.0.69-before-v270-preflight.log
grep -q '"status": "PREFLIGHT_OK"' /tmp/tdh-v2.0.69-before-v270-preflight.log

echo "===== 3. VERIFY REPOSITORY SOURCES ====="
REPO_BRANCH="$(git -C "$REPO" branch --show-current)"
case "$REPO_BRANCH" in
    main|agent/v2-0-70-pre-exhaustion-bridge) ;;
    *)
...
echo "$EXPECTED_TEST_SHA256  $REPO_SOURCE/tests/test_v270_pre_exhaustion_bridge.py" | sha256sum -c -
...
path = sys.argv[1]
spec = importlib.util.spec_from_file_location('tdh_v270_stage_smoke', path)
if spec is None or spec.loader is None:
    raise SystemExit('BLOCKED: v2.0.70 controller import failed')
...
print('V270_PRE_EXHAUSTION_BRIDGE_SMOKE_OK')
PY
