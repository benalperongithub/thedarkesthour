...
SRC="$BASE/v2.0.71"
DST="$BASE/staging/v2.0.72"
TMP="$BASE/staging/.v2.0.72-build-$$"
REPO="/home/tdw/the-darkest-hour"
REPO_SOURCE="$REPO/controller/staging/v2.0.72"
GATE="/usr/local/sbin/tdh-lab-admin-gate"
PYTHON="/srv/tdh-research/phoenix-venv/bin/python"

EXPECTED_CONTROLLER_SHA256="4097306cec4aee7648042c369d1dc0a25bb34c482d8c023049dd996392d3496c"
...
cleanup() {
    if [[ -d "$TMP" && "$TMP" == "$BASE/staging/.v2.0.72-build-"* ]]; then
        rm -rf --one-file-system "$TMP"
    fi
}
trap cleanup EXIT

echo "===== 1. STOP AND BLOCK v2.0.71 DURING UPGRADE ====="
systemctl mask --runtime --now "$SERVICE" || true
sleep 1
test "$(systemctl is-active "$SERVICE" || true)" != "active"

echo "===== 2. VERIFY SEALED v2.0.71 ====="
test -d "$SRC"
...
"$GATE" preflight v2.0.71 >/tmp/tdh-v2.0.71-before-v272-preflight.log
grep -q '"status": "PREFLIGHT_OK"' /tmp/tdh-v2.0.71-before-v272-preflight.log

echo "===== 3. VERIFY REPOSITORY SOURCES ====="
REPO_BRANCH="$(git -C "$REPO" branch --show-current)"
case "$REPO_BRANCH" in
    main|agent/v2-0-72-example-frontier-bridge) ;;
    *)
...
path = sys.argv[1]
spec = importlib.util.spec_from_file_location('tdh_v272_stage_smoke', path)
if spec is None or spec.loader is None:
    raise SystemExit('BLOCKED: v2.0.72 controller import failed')
...
print('V272_EXAMPLE_FRONTIER_BRIDGE_SMOKE_OK')
PY
