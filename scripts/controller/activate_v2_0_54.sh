...
"$GATE" preflight "$NEW_VERSION" >/tmp/tdh-v2.0.54-activation-preflight.log
grep -q '"status": "PREFLIGHT_OK"' /tmp/tdh-v2.0.54-activation-preflight.log
...
[[ "$CMDLINE" == *"$NEW_RELEASE/strategy_lab_controller.py"* ]] || {
    echo "BLOCKED: runtime command is not bound to v2.0.54"
...
[[ "$CMDLINE_AFTER" == *"$NEW_RELEASE/strategy_lab_controller.py"* ]] || {
    echo "BLOCKED: post-bootstrap runtime is not bound to v2.0.54"
