...
SRC="$BASE/v2.0.75"
DST="$BASE/staging/v2.0.76"
TMP="$BASE/staging/.v2.0.76-build-$$"
REPO="/home/tdw/the-darkest-hour"
REPO_SOURCE="$REPO/controller/staging/v2.0.76"
GATE="/usr/local/sbin/tdh-lab-admin-gate"
PYTHON="/srv/tdh-research/phoenix-venv/bin/python"

EXPECTED_CONTROLLER_SHA256="f51eb728dbfa8176882880ff5f0c1e86c0841cacb55788da4ccf696bd6ef793a"
...
cleanup() {
    if [[ -d "$TMP" && "$TMP" == "$BASE/staging/.v2.0.76-build-"* ]]; then
        rm -rf --one-file-system "$TMP"
    fi
}
trap cleanup EXIT

echo "===== 1. STOP AND BLOCK v2.0.75 DURING UPGRADE ====="
systemctl mask --runtime --now "$SERVICE" || true
sleep 1
test "$(systemctl is-active "$SERVICE" || true)" != "active"

echo "===== 2. VERIFY SEALED v2.0.75 ====="
test -d "$SRC"
...
"$GATE" preflight v2.0.75 >/tmp/tdh-v2.0.75-before-v276-preflight.log
grep -q '"status": "PREFLIGHT_OK"' /tmp/tdh-v2.0.75-before-v276-preflight.log

echo "===== 3. VERIFY REPOSITORY SOURCES ====="
REPO_BRANCH="$(git -C "$REPO" branch --show-current)"
case "$REPO_BRANCH" in
    main|agent/v2-0-76-packet-a-global-memory-filter) ;;
    *)
...
path = sys.argv[1]
spec = importlib.util.spec_from_file_location('tdh_v276_stage_smoke', path)
if spec is None or spec.loader is None:
    raise SystemExit('BLOCKED: v2.0.76 controller import failed')
...
print('V276_PACKET_A_GLOBAL_MEMORY_FILTER_SMOKE_OK')
PY
