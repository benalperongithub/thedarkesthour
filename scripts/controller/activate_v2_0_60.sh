...
VERIFY_UNIT="/tmp/strategy-lab-supervisor-v2.1-v260-verify-${STAMP}.service"
PREFLIGHT_LOG="/tmp/tdh-v2.0.60-activation-preflight.log"
UNIT_UPDATED=false
ACTIVATION_MODE=""
...
echo "===== 4. INSTALL UNIT AND START v2.0.60 ====="
if [[ "$ACTIVATION_MODE" == "UPDATE_FROM_OLD" ]]; then
...
[[ "$CMDLINE" == *"$NEW_RELEASE/strategy_lab_controller.py"* ]] || {
    echo "BLOCKED: runtime command is not bound to v2.0.60"
...
[[ "$CMDLINE_AFTER" == *"$NEW_RELEASE/strategy_lab_controller.py"* ]] || {
    echo "BLOCKED: post-bootstrap runtime is not bound to v2.0.60"
...
echo "TDH_V260_ACTIVATION_COMPLETE"
