...
VERIFY_UNIT="/tmp/strategy-lab-supervisor-v2.1-v253-verify-${STAMP}.service"
UNIT_UPDATED=false
...
"$GATE" preflight "$NEW_VERSION" >/tmp/tdh-v2.0.53-activation-preflight.log
grep -q '"status": "PREFLIGHT_OK"' /tmp/tdh-v2.0.53-activation-preflight.log
...
echo "===== 4. INSTALL UNIT AND START v2.0.53 ====="
...
[[ "$CMDLINE" == *"$NEW_RELEASE/strategy_lab_controller.py"* ]] || {
    echo "BLOCKED: runtime command is not bound to v2.0.53"
...
[[ "$CMDLINE_AFTER" == *"$NEW_RELEASE/strategy_lab_controller.py"* ]] || {
    echo "BLOCKED: post-bootstrap runtime is not bound to v2.0.53"
...
echo "TDH_V253_ACTIVATION_COMPLETE"
