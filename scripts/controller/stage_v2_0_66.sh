...
SRC="$BASE/v2.0.65"
DST="$BASE/staging/v2.0.66"
TMP="$BASE/staging/.v2.0.66-build-$$"
REPO="/home/tdw/the-darkest-hour"
REPO_SOURCE="$REPO/controller/staging/v2.0.66"
GATE="/usr/local/sbin/tdh-lab-admin-gate"
PYTHON="/srv/tdh-research/phoenix-venv/bin/python"

EXPECTED_CONTROLLER_SHA256="868949b93f4535ac574097677b160e990fcc7cc2e67aecde29a449a3c0db57f6"
...
"$GATE" preflight v2.0.65 >/tmp/tdh-v2.0.65-before-v266-preflight.log
grep -q '"status": "PREFLIGHT_OK"' /tmp/tdh-v2.0.65-before-v266-preflight.log

echo "===== 3. VERIFY REPOSITORY SOURCES ====="
REPO_BRANCH="$(git -C "$REPO" branch --show-current)"
case "$REPO_BRANCH" in
    main|agent/v2-0-66-frontier-producer-admission) ;;
    *)
...
path = sys.argv[1]
spec = importlib.util.spec_from_file_location('tdh_v266_stage_smoke', path)
if spec is None or spec.loader is None:
    raise SystemExit('BLOCKED: v2.0.66 controller import failed')
...
print('V266_FRONTIER_PRODUCER_ADMISSION_SMOKE_OK')
PY
