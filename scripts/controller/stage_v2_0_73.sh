...
SRC="$BASE/v2.0.72"
DST="$BASE/staging/v2.0.73"
TMP="$BASE/staging/.v2.0.73-build-$$"
REPO="/home/tdw/the-darkest-hour"
REPO_SOURCE="$REPO/controller/staging/v2.0.73"
GATE="/usr/local/sbin/tdh-lab-admin-gate"
PYTHON="/srv/tdh-research/phoenix-venv/bin/python"

EXPECTED_CONTROLLER_SHA256="c494684db750c097f588d15ee1331f5908297f3e135f4a399a340447f5f00383"
...
cleanup() {
    if [[ -d "$TMP" && "$TMP" == "$BASE/staging/.v2.0.73-build-"* ]]; then
        rm -rf --one-file-system "$TMP"
    fi
}
trap cleanup EXIT

echo "===== 1. STOP AND BLOCK v2.0.72 DURING UPGRADE ====="
systemctl mask --runtime --now "$SERVICE" || true
sleep 1
test "$(systemctl is-active "$SERVICE" || true)" != "active"

echo "===== 2. VERIFY SEALED v2.0.72 ====="
test -d "$SRC"
...
"$GATE" preflight v2.0.72 >/tmp/tdh-v2.0.72-before-v273-preflight.log
grep -q '"status": "PREFLIGHT_OK"' /tmp/tdh-v2.0.72-before-v273-preflight.log

echo "===== 3. VERIFY REPOSITORY SOURCES ====="
REPO_BRANCH="$(git -C "$REPO" branch --show-current)"
case "$REPO_BRANCH" in
    main|agent/v2-0-73-example-shape-bridge) ;;
    *)
...
path = sys.argv[1]
spec = importlib.util.spec_from_file_location('tdh_v273_stage_smoke', path)
if spec is None or spec.loader is None:
    raise SystemExit('BLOCKED: v2.0.73 controller import failed')
...
print('V273_EXAMPLE_SHAPE_BRIDGE_SMOKE_OK')
PY
