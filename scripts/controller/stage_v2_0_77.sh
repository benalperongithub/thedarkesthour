...
SRC="$BASE/v2.0.76"
DST="$BASE/staging/v2.0.77"
TMP="$BASE/staging/.v2.0.77-build-$$"
REPO="/home/tdw/the-darkest-hour"
REPO_SOURCE="$REPO/controller/staging/v2.0.77"
GATE="/usr/local/sbin/tdh-lab-admin-gate"
PYTHON="/srv/tdh-research/phoenix-venv/bin/python"

EXPECTED_CONTROLLER_SHA256="0571ce197e606ff786ba1416bebe2da4eb08214a7533fc16b17d8a56b8a6c730"
...
cleanup() {
    if [[ -d "$TMP" && "$TMP" == "$BASE/staging/.v2.0.77-build-"* ]]; then
        rm -rf --one-file-system "$TMP"
    fi
}
trap cleanup EXIT

echo "===== 1. STOP AND BLOCK v2.0.76 DURING UPGRADE ====="
systemctl mask --runtime --now "$SERVICE" || true
sleep 1
test "$(systemctl is-active "$SERVICE" || true)" != "active"

echo "===== 2. VERIFY SEALED v2.0.76 ====="
test -d "$SRC"
...
"$GATE" preflight v2.0.76 >/tmp/tdh-v2.0.76-before-v277-preflight.log
grep -q '"status": "PREFLIGHT_OK"' /tmp/tdh-v2.0.76-before-v277-preflight.log

echo "===== 3. VERIFY REPOSITORY SOURCES ====="
REPO_BRANCH="$(git -C "$REPO" branch --show-current)"
case "$REPO_BRANCH" in
    main|agent/v2-0-77-sealed-implementation-queue) ;;
    *)
...
path = sys.argv[1]
spec = importlib.util.spec_from_file_location('tdh_v277_stage_smoke', path)
if spec is None or spec.loader is None:
    raise SystemExit('BLOCKED: v2.0.77 controller import failed')
...
print('V277_SEALED_IMPLEMENTATION_QUEUE_SMOKE_OK')
PY
